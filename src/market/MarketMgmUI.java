...
public class MarketMgmUI extends JFrame {
   ...
   //Method
      public void showMain() {  //10.13 영재 수정
         //영화
    	  icon = new ImageIcon("C:\\dev\\eclipse_workspace\\market\\images\\back.png");
    	  JPanel background = new JPanel()  {
    		  public void paintComponent(Graphics g) {
    			  g.drawImage(icon.getImage(), 0, 0, this.getWidth(), this.getHeight(), this);	//화면크기에 따라 이미지 크기 변경
...
//         jl_title.setFont(new Font("맑은 고딕",Font.BOLD,16));
         btnLogin.setFont(new Font("바탕", Font.BOLD, 12));
         ...
         getContentPane().add(showPane, BorderLayout.NORTH);
         getContentPane().add(showButtonPane, BorderLayout.SOUTH);
         
         //영화
//           background.add(showPane, BorderLayout.CENTER);
//         background.add(showButtonPane, BorderLayout.SOUTH);

         getContentPane().add(background);
         JPanel jp_login=new JPanel(new GridLayout(2,1));
         background.add(jp_login);
...
//         contentPane.add(showPane, BorderLayout.SOUTH);
//         contentPane.add(showButtonPane, BorderLayout.SOUTH);
